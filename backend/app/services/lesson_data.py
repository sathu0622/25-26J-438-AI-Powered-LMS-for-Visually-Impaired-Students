...
LESSON_DATA: Dict[int, List[dict]] = {
    10: [
        {
            "id": 1,
            "grade": 10,
            ...
            "topics": [
                {
                    ...
                    "content": """Ancient Sri Lanka made extraordinary contributions to Buddhist literature. The most significant achievement was the writing down of the Tripitaka (Buddhist canon) in Pali during the reign of King Valagamba in the 1st century BCE, preserving Buddha's teachings for posterity. The Mahavamsa, written by Mahanama Thera in the 6th century CE, is a historical chronicle of exceptional literary merit. Commentaries on Buddhist texts, such as those by Buddhaghosa, were composed in Sri Lanka. Sinhala literature flourished with works like the Sikhavalaya and Muvadevdavata. Poetic works describing pilgrimage sites and Buddhist teachings demonstrated sophisticated literary development. These texts were copied and preserved in monasteries, creating a strong textual tradition.""",
                    "duration": "150"
                },
                {
                    "id": 20,
            ...
            "topics": [
                {
                    ...
                    "content": """The Portuguese arrived in Sri Lanka in 1505 when a fleet commanded by Lourenco de Almeida was blown off course and reached Colombo. Initially received as traders, the Portuguese quickly established their first fort in Colombo in 1517. Their arrival marked the beginning of European colonialism in Sri Lanka. The Portuguese were primarily interested in the cinnamon trade and controlling strategic ports in the Indian Ocean trade network. They employed a strategy of making alliances with local kingdoms while gradually expanding their control. By the mid-16th century, they controlled most coastal areas, though they never conquered the Kandyan Kingdom in the interior. Portuguese rule lasted until 1658 when they were displaced by the Dutch.""",
                    "duration": "145"
                },
                {
                    "id": 34,
...
def get_grades() -> List[int]:
    return sorted(LESSON_DATA.keys())


def get_lessons_by_grade(grade: int) -> List[Lesson]:
    lessons = LESSON_DATA.get(grade, [])
    return [Lesson(**lesson) for lesson in lessons]


def get_lesson_by_id(lesson_id: int) -> Optional[Lesson]:
    for grade_lessons in LESSON_DATA.values():
...
def get_topics_by_lesson_id(lesson_id: int) -> List[Topic]:
    for grade_lessons in LESSON_DATA.values():
        for lesson in grade_lessons:
            if lesson.get("id") == lesson_id:
                return [Topic(**topic) for topic in lesson.get("topics", [])]
    return []
