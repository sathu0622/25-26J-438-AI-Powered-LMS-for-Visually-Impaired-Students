...
function ResultsDisplay({ results }) {
  ...
  // Safely handle the summaries array
    ...
    return results.summaries.map((summary, index) => (
      ...
      >
        ...
        <div className="space-y-3">
          {summary.headline && (
            <h4 className="font-bold text-gray-800 text-lg">{summary.headline}</h4>
          )}
          ...
          <p className="text-gray-700 leading-relaxed text-base">
            {typeof summary === 'object' ? summary.summary || 'No summary available' : summary}
          </p>
        </div>
      </div>
    ));
